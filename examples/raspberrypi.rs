...
fn main() {
    ...
    let address = 0x68;
    let mut adc = MCP3425::new(dev, address, Delay).with_resolution(Resolution::SPS240Bits12);

    println!("Temperature 12 bit / 1x gain: {:?}", adc.oneshot().map_err(|e| format!("{:?}", e)));
    adc.set_resolution(Resolution::SPS60Bits14);
    println!("Temperature 14 bit / 1x gain: {:?}", adc.oneshot().map_err(|e| format!("{:?}", e)));
    adc.set_resolution(Resolution::SPS15Bits16);
